...
ggplot(dt, aes (x= Liver)) +
  geom_histogram(breaks = seq(0, 100, 2), position = "stack") +
  labs(
    x = "TPM",
    y = "Gene expression of liver"
  )
# answer here
#The distribution seems skewed heavily to the right because most genes in the liver are expressed at a small level while a subset of genes like specific mitochondrial genes may be highly expressed. I would not be surprised if this was the pattern for cells outside of the liver because most cells in tissues are expressed at a low level. 
```
...
# answer here
ggplot(dt, aes (x= Liver)) +
  geom_density(breaks = seq(0, 100, 2), position = "stack") +
  labs(
    x = "TPM",
    y = "Gene expression of liver"
  ) +
...
ggplot(dt, aes(x = Liver + 1, y = `Heart - Left Ventricle` + 1)) +
  ...
  scale_color_brewer(palette = "Set2", name = "Name") +
  theme_minimal()
```
...
mito <- dt %>%
  ...
  mutate(is_mitochondria = str_starts(dt$Description,"MT"))

ggplot(mito, aes(x = Liver + 1, y = `Heart - Left Ventricle` + 1, color = is_mitochondria)) +
  ...
  scale_x_log10() + 
  scale_y_log10() +
  scale_color_brewer(palette = "Set1", name = "Name") +
  theme_minimal()
# answer here
```
...
```{r}
dt_long <- dt %>%
  mutate(is_mitochondrial = str_starts(dt$Description,"MT")) %>% 
